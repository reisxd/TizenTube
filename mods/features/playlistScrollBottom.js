...
function continuationToCommand(item) {
  if (!item || typeof item !== 'object') return null;
  if (item.continuationEndpoint) return item.continuationEndpoint;
  const next = item.nextContinuationData;
  if (next?.continuation) {
    return {
      clickTrackingParams: next.clickTrackingParams || null,
...
function storeContinuationCommands(r) {
  try {
    const rendererContinuations = r?.contents?.tvBrowseRenderer?.content?.tvSurfaceContentRenderer?.content?.twoColumnRenderer?.rightColumn?.playlistVideoListRenderer?.continuations || [];
    const continuationContinuations = r?.continuationContents?.playlistVideoListContinuation?.continuations || [];
    const commands = [];
    for (const raw of [...rendererContinuations, ...continuationContinuations]) {
      const cmd = continuationToCommand(raw);
      if (cmd) commands.push(cmd);
    }
    if (commands.length) {
      window.__ttPlaylistContinuationCommands = commands;
      _log('playlist.scroll.continuations.stored', { count: commands.length });
    }
  } catch (err) {
    _log('playlist.scroll.continuations.error', { msg: String(err?.message || err) });
  }
}
...
export function playlistScrollBottom(resolveCommandFn, showToastFn) {
  try {
    const continuationCmd = Array.isArray(window.__ttPlaylistContinuationCommands) ? window.__ttPlaylistContinuationCommands[0] : null;
    if (continuationCmd && typeof resolveCommandFn === 'function') {
      _log('playlist.scroll.continuation.run', {});
      resolveCommandFn(continuationCmd);
    }
...
JSON.parse = function () {
  ...
  try {
    ...
    if (isPlaylist) storeContinuationCommands(r);
